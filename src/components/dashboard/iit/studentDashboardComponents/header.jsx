...
const Header = ({ showNotifications, setShowNotifications, toggleSidebar }) => {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      ...
      setLoading(false); // End loading in both cases
    });
  ...
  const handleLogout = async () => {
    try {
      ...
      window.location.replace("/iit-role-login"); // Redirect to IIT Portal Access
    } catch (error) {
  ...
  // 🔴 If not authenticated, redirect to IIT Portal Access
  if (!user) {
                ...
                <div className="max-h-60 overflow-y-auto">
                  <div className="px-4 py-3 hover:bg-gray-50 border-b border-gray-100">
                    ...
                    <p className="text-xs text-gray-500">
                      OOP class is canceled tomorrow.
                    </p>
